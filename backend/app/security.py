...
EMAIL_VERIFY_EXPIRES_MIN = int(os.getenv("EMAIL_VERIFY_EXPIRES_MIN", "1440"))  # 24h
...
def create_email_verify_token(sub: str, email: str) -> str:
    exp = datetime.utcnow() + timedelta(minutes=EMAIL_VERIFY_EXPIRES_MIN)
    return jwt.encode(
        {"sub": sub, "email": email, "exp": exp, "typ": "email_verify"},
        JWT_SECRET,
        algorithm="HS256",
    )
